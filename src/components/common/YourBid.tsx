...
const YourBid: React.FC<YourBidProps> = ({ bid, isProjectOpen, onUpdate, onWithdraw, loading = false }) => {
    ...
    const handleEditClick = () => {
        if (bid) {
            setEditData({
                ...
                deliveryTime: String(bid.deliveryTime),
                message: bid.message
            });
            setIsEditing(true);
        }
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (bid && onUpdate) {
            await onUpdate(bid.id, {
                bidAmount: Number(editData.bidAmount),
                deliveryTime: Number(editData.deliveryTime),
    ...
    if (!bid) {
        return (
            <div className="bg-[#111118] rounded-[2.5rem] border border-white/5 p-8 md:p-12 shadow-2xl">
                <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-6">Your Bid</h2>
        ...
        return (
            <div className="bg-[#111118] rounded-[2.5rem] border border-white/5 p-8 md:p-12 shadow-2xl">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Your Bid</h2>
                ...
                <form onSubmit={handleEditSubmit} className="space-y-5">
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            onChange={(e) => setEditData({ ...editData, bidAmount: e.target.value })}
                            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all text-sm"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            rows={4}
                            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all resize-none text-sm"
                        />
                    </div>
